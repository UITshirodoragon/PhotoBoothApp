...
class Image_Capture_Interface(ctk.CTkFrame):
    ...
    def Take_gif(self):
        ...
        if self.controller.gif_image_count < 75:
            ...
            self.controller.gif_image_count = 0
            self.controller.gif_count += 1
            self.controller.list_gif_image[0].save(f'DataStorage/GIFGallery/user{self.end_screen.controller.number_of_uses}/Gif{self.controller.gif_count}.gif',
                                        save_all = True,
            ...
            self.Notification_label.place(relx = 0.5, rely = 0.5, anchor = 'center') # layout nofitication
            self.Notification_label.after(500, self.Notification_label.place_forget) # close the nofitication
            self.controller.just_captured_gif_path = f'DataStorage/GIFGallery/user{self.end_screen.controller.number_of_uses}/Gif{self.controller.gif_count}.gif'
    ...
    def Take_Picture(self):
        ...
        if ret:
            ...
            self.Notification_label.configure(text = 'Captured successfully')
            cv2.imwrite(f'DataStorage/ImageGallery/user{self.end_screen.controller.number_of_uses}/image{self.controller.Captured_numbers}.png', frame) # Save image
            #Tell that an image is captured
            self.controller.is_captured_yet = True
            self.controller.just_captured_image_path = f'DataStorage/ImageGallery/user{self.end_screen.controller.number_of_uses}/image{self.controller.Captured_numbers}.png'
        else:
            self.Notification_label.configure(text = 'Captured unsuccessfully')
        self.Notification_label.place(relx = 0.5, rely = 0.5, anchor = 'center') # layout nofitication
        self.Notification_label.after(500, self.Notification_label.place_forget) # close the nofitication
